...
fn compute(content: String) -> String {
    ...
    let mut paths: Vec<Vec<Vec<usize>>> = content
        .trim_end()
        .split('\n')
        .map(|path| {
            path.split(" -> ")
    ...
    let mut cave = vec![vec![false; cave_shape.1 + 3]; cave_shape.0 + cave_shape.1 + 3];
    // Initialize the paths in the cave
    paths.push(vec![
        vec![0, cave_shape.1 + 2],
        vec![cave.len() - 1, cave_shape.1 + 2],
    ]);
    for path in paths {
